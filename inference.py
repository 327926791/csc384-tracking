...
import busters
import game
from pip._vendor.requests.packages.chardet import chardistribution
    ...
    def observe(self, observation, gameState):
        ...
        * When a ghost is captured by Pacman, all beliefs should be
        ...
        for p in self.legalPositions:
            ...
            # get emissinModel, and calculate the distribution for new Pos
            trueDistance = util.manhattanDistance(p, pacmanPosition)
            if emissionModel[trueDistance] > 0:
                allPossible[p] = 1.0
            
            allPossible[p] = self.beliefs[p] * emissionModel[trueDistance] * allPossible[p] 
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for i in range(self.numParticles):
            self.particles.append(self.legalPositions[i % total_pos])
    ...
    def getBeliefDistribution(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        for i in set(self.particles):
            self.beliefs[i] = self.particles.count(i)*1.0/self.numParticles
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        for p in itertools.product(self.legalPositions, repeat=self.numGhosts):
            self.ghost_pos.append(p)
      
        total_pos = len(self.ghost_pos)
        for i in range(self.numParticles):
            self.particles.append(self.ghost_pos[i % total_pos])            
        "*** END YOUR CODE HERE ***"
    ...
    def observeState(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        weights=[]
        pos_list = self.ghost_pos
        
        # for each ghost, get emissionModel and weight value   
        for ghost_i in range(self.numGhosts):
            emissionModel = emissionModels[ghost_i]
            
            allPossible = util.Counter()
            weight = util.Counter()
            # check if ghost in jail
            # if yes, set weigh of this pos to 1, others to 0
            # set allPossible of this pos to 1, others to 0
            if noisyDistances[ghost_i] == None:
                jail_p = self.getJailPosition(ghost_i)
                new_Particles = util.Counter()
                pos_list=[]
                for i in range(len(self.particles)):
                    self.particles[i] = self.getParticleWithGhostInJail(self.particles[i], ghost_i)
                    if self.particles[i] not in pos_list:
                        pos_list.append(self.particles[i])
            ...
            total_wt = 0
            # if not in jail
            # get the pb for all legalPosition with noisyDis
            # weight = pb with noisyDistance
            for p in self.legalPositions:
                trueDistance = util.manhattanDistance(p, pacmanPosition)
                weight[p] = emissionModel[trueDistance]
            weights.append(weight)          
        
        # calculate weight for all possible positions of ghost      
        weight = util.Counter()
        total_wt = 0
        for p in pos_list:
            i= 0
            weight[p] = 1
            for pp in list(p):
                weight[p] = weight[p] * weights[i][pp]
                i = i + 1
            
            total_wt = total_wt + self.particles.count(p) * weight[p] 
        if total_wt == 0:
            self.initializeParticles()
            return
        
        # calculate probability for all possible positions    
        for p in pos_list:
            allPossible[p] = self.particles.count(p) * weight[p] / total_wt
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            # for each ghost, get its distribution of next position
            for i in range(self.numGhosts):
                distribution = util.Counter()
                newPosDist = getPositionDistributionForGhost(setGhostPositions(gameState, newParticle), i, self.ghostAgents[i])
                newPosDists.append(newPosDist)
                
                # if there are more than one ghost
                if (i>0):
                    for p2 in newPosDists[i]:
                        for p1 in newPosDists[i-1]:
                            if i-1 > 0:
                                p = list(p1)
                            else:
                                p = list([p1])
                            p.append(p2)
                            new_p = tuple(p)
                            distribution[new_p] = newPosDists[i][p2] * newPosDists[i-1][p1]
                # if only one ghost
                else:
                    distribution = newPosDist
                    
            #re-sample particle according to distribution
            newParticle= util.sample(distribution)
